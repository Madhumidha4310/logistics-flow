...
import {
  FiTruck, FiMapPin, FiBarChart2, FiShield, FiClock, FiPackage, FiArrowRight, FiCheckCircle,
} from "react-icons/fi";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Run your fleet, bookings, drivers, and live shipments from one beautiful command center." },
    ],
  }),
  component: Landing,
});
...
const features = [
  { icon: FiTruck, title: "Fleet Management", desc: "Track every vehicle, maintenance log, and assignment in real time." },
  { icon: FiMapPin, title: "Live Tracking", desc: "Pinpoint shipments on the map and share ETAs with customers automatically." },
  { icon: FiPackage, title: "Smart Bookings", desc: "Create, dispatch, and invoice shipments in seconds with built-in workflows." },
  { icon: FiBarChart2, title: "Analytics", desc: "Revenue, utilization, and driver KPIs in clean, interactive dashboards." },
  { icon: FiShield, title: "Role-Based Access", desc: "Super Admin, Admin, Manager and Customer roles with granular permissions." },
  { icon: FiClock, title: "24/7 Operations", desc: "Push notifications and SLA alerts keep your team ahead of every delivery." },
];
          ...
          {user ? (
            ...
            <>
              ...
              <Button asChild className="bg-gradient-accent text-white hover:opacity-95">
                <Link to="/signup">Start free <FiArrowRight className="ml-1" /></Link>
              </Button>
            </>
          )}
        </div>
    ...
    <div className="min-h-screen">
          ...
          >
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Button asChild size="lg" className="bg-white text-[oklch(0.24_0.07_240)] hover:bg-white/90 shadow-glow">
                <Link to="/signup">Get started free <FiArrowRight className="ml-2" /></Link>
              </Button>
            ...
            <ul className="mt-8 grid sm:grid-cols-2 gap-3 text-sm text-white/75">
              {["No credit card required", "Setup in under 5 minutes", "Role-based access control", "Live shipment ETAs"].map((t) => (
                <li key={t} className="flex items-center gap-2"><FiCheckCircle className="text-[oklch(0.78_0.10_195)]" /> {t}</li>
              ))}
            </ul>
          </motion.div>
          ...
          <div className="mt-14 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <motion.div
                ...
                className="group glass rounded-2xl p-6 shadow-soft hover:shadow-glow transition"
              >
                <div className="size-12 rounded-xl bg-gradient-accent grid place-items-center text-white shadow-glow group-hover:scale-105 transition">
      ...
      <section id="cta" className="pb-24">
        <div className="mx-auto max-w-6xl px-6">
          <div className="relative rounded-3xl overflow-hidden bg-gradient-hero shadow-glow p-12 md:p-16 text-white text-center">
            <div className="absolute inset-0 [background:radial-gradient(60%_60%_at_50%_50%,oklch(0.78_0.10_195/.25),transparent_70%)]" />
            ...
            <p className="relative mt-4 text-white/75 max-w-xl mx-auto">
              Start free today. Onboard your fleet in minutes, and unlock real-time visibility across every booking.
            </p>
            <div className="relative mt-8 flex flex-wrap justify-center gap-3">
              <Button asChild size="lg" className="bg-white text-[oklch(0.24_0.07_240)] hover:bg-white/90">
                <Link to="/signup">Create free account <FiArrowRight className="ml-2" /></Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="bg-transparent border-white/30 text-white hover:bg-white/10">
                <Link to="/login">Sign in</Link>
              </Button>
      ...
      <footer className="border-t border-border py-10">
        <div className="mx-auto max-w-7xl px-6 flex flex-wrap items-center justify-between gap-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            <div className="size-7 rounded-lg bg-gradient-accent grid place-items-center">
              <FiTruck className="text-white text-sm" />
            </div>
            <span className="font-display font-semibold text-foreground">Transport Agencies</span>
          </div>
          <p>© {new Date().getFullYear()} Transport Agencies. All rights reserved.</p>
        </div>
      </footer>
    </div>
  );
